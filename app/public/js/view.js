...
$("#search-btn").on("click", function() {
  ...
  $.get("/api/" + searchedCharacter, function(data) {
    ...
    if (!data) {
      ...
      $("#well-section").append("<h2>" + data[0].name + "</h2>");
      // the role
      $("#well-section").append("<h3>Role: " + data[0].role + "</h3>");
      // the age
      $("#well-section").append("<h3>Age: " + data[0].age + "</h3>");
      // and the force points
      $("#well-section").append("<h3>Force Points: " + data[0].forcePoints + "</h3>");
    }
  });
});
